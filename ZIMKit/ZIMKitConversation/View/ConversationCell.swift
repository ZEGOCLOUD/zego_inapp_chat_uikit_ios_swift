...
class ConversationCell: _TableViewCell {
    ...
    override func updateContent() {
        ...
        headImageView.loadImage(with: model.conversationAvatarUrl, placeholder: placeHolder)

        let userName = model.conversationName.count > 0
        ...
        if model.lastMessage?.timestamp == 0 {
            ...
            timeLabel.text = timestampToConversationDateStr(model.lastMessage?.timestamp)
        }
        ...
        msgFailImageView.isHidden = model.lastMessage?.sentStatus != .sendFailed

        if model.lastMessage?.sentStatus == .sendFailed {
