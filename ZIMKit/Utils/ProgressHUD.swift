...
public extension ProgressHUD {
    ...
    // -------------------------------------------------------------------------------------------------------------------------------------------
    class func dismiss(_ immediately: Bool = false) {

        DispatchQueue.main.async {
            if immediately {
                shared.hudHideImmediately()
            } else {
                shared.hudHide()
            }
        }
    ...
    // -------------------------------------------------------------------------------------------------------------------------------------------
    class func show(_ status: String? = nil, hide: Bool = false, interaction: Bool = true) {

        DispatchQueue.main.async {
            shared.setup(status: status, hide: hide, interaction: interaction)
        }
    }
...
public class ProgressHUD: UIView {
    ...
    private var fontStatus		= UIFont.systemFont(ofSize: 13)
    ...
    private func setup(status: String? = nil, progress: CGFloat? = nil, staticImage: UIImage? = nil, hide: Bool, interaction: Bool) {
        ...
        let onlyText = animationType == .none && progress == nil && staticImage == nil

        setupSize(onlyText, progress != nil)
        setupPosition()
    ...
    private func setupBackground(_ interaction: Bool) {
        ...
        if viewBackground == nil {
            let mainWindow = UIApplication.key ?? UIWindow()
    ...
    private func setupProgress(_ progress: CGFloat?) {
        ...
        if viewProgress == nil {
            ...
            viewProgress?.frame = CGRect(x: 0, y: 0, width: 50, height: 50)
        }
    ...
    private func setupStaticImage(_ staticImage: UIImage?) {
        ...
        if staticImageView == nil {
            staticImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 40, height: 40))
        }
    ...
    // MARK: -
    // -------------------------------------------------------------------------------------------------------------------------------------------
    private func setupSize(_ onlyText: Bool, _ isProgress: Bool = false) {

        let width: CGFloat = isProgress ? 140 : 130
        var height: CGFloat = isProgress ? 135 : 125

        if let text = labelStatus?.text {
            let sizeMax = CGSize(width: width-24, height: 1000)
            ...
            //            width = ceil(rectLabel.size.width) + 24
            height = ceil(rectLabel.size.height) + 60 + 8 + (isProgress ? 50 : 40)
            if onlyText {
                height = rectLabel.height + 14 * 2
            }

            rectLabel.origin.x = 12.0
            rectLabel.origin.y = onlyText ? 14 : (isProgress ? 88 : 78)
            rectLabel.size.width = width - 24
            rectLabel.size.height = max(17, rectLabel.height)
    ...
    private func keyboardHeight() -> CGFloat {
        ...
        if let keyboardWindowClass = NSClassFromString("UIRemoteKeyboardWindow"),
           ...
           let inputSetHostView = NSClassFromString("UIInputSetHostView") {
            ...
            where window.isKind(of: keyboardWindowClass) {
                for firstSubView in window.subviews
                where firstSubView.isKind(of: inputSetContainerView) {
                    for secondSubView in firstSubView.subviews
                    where secondSubView.isKind(of: inputSetHostView)  {
    ...
    private func hudShow() {
        ...
        if self.alpha != 1 {
            ...
            UIView.animate(withDuration: 0.15, delay: 0, options: [.allowUserInteraction, .curveEaseIn], animations: {
                self.toolbarHUD?.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
    ...
    // -------------------------------------------------------------------------------------------------------------------------------------------
    private func hudHideImmediately() {
        if self.alpha == 1 {
            self.toolbarHUD?.alpha = 0
            self.hudDestroy()
            self.alpha = 0
...
private class ProgressView: UIView {
    ...
    func setupLayers() {
        ...
        labelPercentage.font = .systemFont(ofSize: 13)
