...
class GalleryVC: _ViewController {
    ...
    struct Content {
        public var messageViewModels: [MessageViewModel]
        public var currentMessageVM: MessageViewModel
        public var currentIndex: Int

        public init(messageViewModels: [MessageViewModel], currentMessageVM: MessageViewModel, index: Int) {
            self.messageViewModels = messageViewModels
            self.currentMessageVM = currentMessageVM
    ...
    var items: [MessageViewModel] {
        let msgs = content.messageViewModels.filter({ $0.message.type == .image })
        return msgs
    }
    ...
    @objc func downloadButtonClick(_ sender: UIButton) {
        guard let messageVM = content.currentMessageVM as? ImageMessageViewModel else { return }
    ...
    func updateCurrentPage() {
        ...
        content.currentMessageVM = items[content.currentIndex]
    }
...
extension GalleryVC: UICollectionViewDataSource,
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.messageVM = items[indexPath.row]
